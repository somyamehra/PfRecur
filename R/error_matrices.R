...
#' @param EPSILON Genotyping error rate (in the range [0, 1])
...
geometric_error_matrix <- function(marker_set, EPSILON) {
  ...
  if (EPSILON<0 | EPSILON>1) stop("EPSILON must lie in the range [0, 1]")

  if (!all(sapply(marker_set, function(x) all(!grepl("[^0-9]", x))))) {
    stop("This error matrix can only be generated for length-polymorphic markers with integer allele lengths")
  }
